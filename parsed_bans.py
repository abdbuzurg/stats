...
matches_bans = []
print("Forming the list for heroes that's have been banned")
current_time = time.localtime(time.time())
print("Started at {0}:{1}:{2}".format(current_time.tm_hour,current_time.tm_min, current_time.tm_sec))
k = 0
for i in range(0, len(picks_bans)):
    match_id = picks_bans[i]["match_id"]
    if (match_id in matches_id_for_patch and match_id in matches_id_for_win):
        ...
        index_of_hero = hero_ids.index(picks_bans[i]["hero_id"])
        ...
        if (i / len(picks_picks) > 0.8 and k == 3): 
            ...
            print("Started at {0}:{1}:{2}".format(current_time.tm_hour,current_time.tm_min, current_time.tm_sec))
            k += 1
        matches_bans.append(match)

i = 0
current_patch = matches_bans[i]["patch"]
current_patch = list(current_patch)
del current_patch[1]
current_patch = ''.join(current_patch)
print("Starting the creation of CSVs for banned heroes in specific patch")
...
while i < len(matches_bans):
    
    with open("parsed_banrates" + current_patch + ".csv", mode = "w") as File:
        fieldnames = matches_bans[0].keys()
        writer = csv.DictWriter(File, fieldnames=fieldnames)

        writer.writeheader()
        j = 0
        for k in range(i, len(matches_bans)):
            patch = matches_bans[k]["patch"]
            patch = list(patch)
            del patch[1]
            patch = ''.join(patch)
            if (patch == current_patch): 
                writer.writerow({"hero": matches_bans[k]["hero"]})
                j += 1
                
            else:
                print("Finishing the csv about patch " + current_patch)
                current_patch = matches_bans[k]["patch"]
                current_patch = list(current_patch)
                del current_patch[1]
                current_patch = ''.join(current_patch)
